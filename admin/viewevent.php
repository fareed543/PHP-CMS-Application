...
$count_que="select count(*) from wn_event";
...
if(isset($_GET["va"])){
	switch($_GET["va"]){
		case '1':
			...
			break;
		case '2':
			$message="Selected Event Modified";
			break;
		case '3':
...
$strSql="select * from wn_event order by $sortby $disType limit $limit1,$pagelimit";
...
function fnAdd()
{
	window.location.href="editevent.php?tType=Add";
}
function fnDetails()
{
	...
	{
		window.location.href="editevent.php?tType=Edit&pg=<?=$pg?>&uid="+arrval[0];
	}
	}
...
{	
	...
	}else if(flag==1)
	{
			var i,len,chkdelids,sep;
			  ...
			  sep="";
				...
				ConfirmStatus = confirm("Do you want to DELETE selected event?")

                if (ConfirmStatus == true) {

				document.frmMain.chkdelids.value=chkdelids
				document.frmMain.tType.value="Del"
				document.frmMain.action="submitevent.php";
  ...
  <tr>
    <td width="25%" height="40" align="left" valign="middle"><table border="0" cellspacing="0" cellpadding="0" style="margin-left:30px;">
	  <tr>
        <td height="40" ><span class="innerpageheading">Event Manager : </span><span class="innerpageheading1">Event</span></td>
      </tr>
    </table></td>
    ...
    <td height="100%" colspan="3" align="center" valign="top"><table width="96%" height="100%" border="0" cellpadding="0" cellspacing="0">
      <tr>
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0">
          <tr>
            <td height="25" bgcolor="F5F5F5"><table width="100%" height="25" border="0" cellpadding="1" cellspacing="1">
              <tr>
                <td width="25%" align="left" bgcolor="#F5F5F5" style="padding-left:25px;"><strong>Event ( <?=$total_count?> )</strong> </td>
		  ...
		  <tr>
            <td valign="top"><table width="100%" border="0" cellpadding="5" cellspacing="1" bgcolor="#E2E2E2" style="border-bottom:1px solid #D2001E;">
              ...
              <tr>
				...
				<input type="checkbox" name="selectcheck" value="<?=$rs["e_id"]?>"/></td>
                <td height="20" bgcolor="<?=$color?>" style="padding-left:10px;" align="left"><?=$rs["e_name"]?></td>
		...
		<input type="hidden" name="chkdelids">
          ...
          <tr>
            <td height="25" align="center" bgcolor="#F5F5F5"><?if($pg>1){?><a href="viewevent.php?pg=1&disType=<?=$disType1?>&sortby=<?=$sortby?>" class="link3">&lt;&lt; Start</a><?}else{?><span class="link3">&lt;&lt; Start</span><?}?>&nbsp;&nbsp;<?if($pg>1){?><a href="viewevent.php?pg=<?=$pg-1?>&disType=<?=$disType1?>&sortby=<?=$sortby?>" class="link3">&lt; Previous&nbsp;</a><?}else{?><span class="link3">&lt; Previous&nbsp;</span><?}?> | <?if($intpagecount>$pg){?>&nbsp;<a href="viewevent.php?pg=<?=$pg+1?>&disType=<?=$disType1?>&sortby=<?=$sortby?>" class="link3">Next &gt;</a><?}else{?>&nbsp;<span class="link3">Next &gt;</span><?}?> <?if($intpagecount>$pg){?>&nbsp;<a href="viewevent.php?pg=<?=$intpagecount?>&disType=<?=$disType1?>&sortby=<?=$sortby?>" class="link3">End &gt;&gt;</a><?}else{?>&nbsp;<span class="link3">End &gt;&gt;</span><?}?></td>
          </tr>
		  <?}else{?>
		  <tr>
