...
include("includes/functions.php");
        ...
        $dprice	    =  $_REQUEST["dprice"];
         $price_e	    =  $_REQUEST["price_e"];
          $price_uk	    =  $_REQUEST["price_uk"];
           $price_nz	    =  $_REQUEST["price_nz"];
            $price_si	    =  $_REQUEST["price_si"];
             $price_aus	    =  $_REQUEST["price_aus"];
              $p_disc	    =  $_REQUEST["p_disc"];
		$qty 	    = $_REQUEST["qty"];
		$desc       = $_REQUEST["desc"];
...
{   
		...
		$insert_p_color_quaey="insert into wn_products (cat_id,subcat_id,prod_title,prod_image,prod_desc,prod_cuntry,p_event,uk_access,quantity,price,price_d,price_e,price_uk,price_nz,price_aus,price_si,p_disc,uk_custservice,national_calls,local_calls,free_calls,new_stock,discount,prod_vill) values ('1', '".$cat_id."', '".addslashes($prod_name)."',  '".addslashes($image3)."', '".addslashes($desc)."','".$selcountry."', '".$e_id."' ,'".addslashes($uk_access)."', '".addslashes($qty)."', '".addslashes($price)."', '".addslashes($dprice)."', '".addslashes($price_e)."', '".addslashes($price_uk)."', '".addslashes($price_nz)."', '".addslashes($price_aus)."', '".addslashes($price_si)."', '".addslashes($p_disc)."','".addslashes($uk_custservice)."','".addslashes($national_calls)."','".addslashes($local_calls)."','".addslashes($free_calls)."','".addslashes($new_stock)."', '".addslashes($discount)."','".$selcountry1."')";
	...
	$update_event_quaey="update wn_products set subcat_id='".$cat_id."', prod_title='".addslashes($prod_name)."', prod_image='".$image3."',prod_cuntry='".$selcountry."', prod_desc='".addslashes($desc)."', quantity='".addslashes($_REQUEST[qty])."', price='".addslashes($_REQUEST[price])."', price_d='".addslashes($_REQUEST[dprice])."', price_e='".addslashes($_REQUEST[price_e])."', price_uk='".addslashes($_REQUEST[price_uk])."', price_nz='".addslashes($_REQUEST[price_nz])."', price_aus='".addslashes($_REQUEST[price_aus])."',price_si='".addslashes($_REQUEST[price_si])."' ,p_disc='".addslashes($_REQUEST[p_disc])."', P_event='".addslashes($_REQUEST[e_id])."' , new_stock ='".addslashes($new_stock)."' , prod_vill ='".$selcountry1."'  where prod_id= ".$_REQUEST["uid"];
