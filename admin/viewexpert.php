...
$count_que="select count(*) from wn_expert";
...
if(isset($_GET["va"])){
	switch($_GET["va"]){
		case '1':
			...
			break;
		case '2':
			$message="Selected Expert Report Modified";
			break;
		case '3':
			$message="Selected Expert Report Deleted";
			break;
		case '4':
...
if(!isset($sortby)) { $sortby="photo_id";}
...
$strSql="select * from wn_expert order by $sortby $disType limit $limit1,$pagelimit";
...
function fnAdd()
{
	window.location.href="editexpert.php?tType=Add";
}
function fnDetails()
{
	...
	{
		window.location.href="editexpert.php?tType=Edit&pg=<?=$pg?>&uid="+arrval[0];
	}
	}
...
{	
	...
	}else if(flag==1)
	{
			var i,len,chkdelids,sep;
			  ...
			  sep="";
				...
				ConfirmStatus = confirm("Do you want to DELETE selected image?")

                if (ConfirmStatus == true) {

				document.frmMain.chkdelids.value=chkdelids
				document.frmMain.tType.value="Del"
				document.frmMain.action="submitexpert.php";
  ...
  <tr>
    <td width="25%" height="40" align="left" valign="middle"><table border="0" cellspacing="0" cellpadding="0" style="margin-left:30px;">
	  <tr>
        <td height="40" ><span class="innerpageheading">Content Management : </span><span class="innerpageheading1">Our Experts</span></td>
      </tr>
    </table></td>
    ...
    <td height="100%" colspan="3" align="center" valign="top"><table width="96%" height="100%" border="0" cellpadding="0" cellspacing="0">
      <tr>
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0">
          <tr>
            <td height="25" bgcolor="F5F5F5"><table width="100%" height="25" border="0" cellpadding="1" cellspacing="1">
              <tr>
                <td width="25%" align="left" bgcolor="#F5F5F5" style="padding-left:25px;"><strong>Our Experts ( <?=$total_count?> )</strong> </td>
		  ...
		  <tr>
            <td valign="top"><table width="100%" border="0" cellpadding="5" cellspacing="1" bgcolor="#E2E2E2" style="border-bottom:1px solid #D2001E;">
              <tr>
                ...
                <td height="45%" align="center" valign="middle" background="images/tab_img_bg.gif" class="tableheading"><a href="<?=$myurl."&sortby=title"?>" class="link3">Title</a> </td>
			  ...
			  <?
				...
				?>
              <tr>
                <td height="35" align="center" bgcolor="<?=$color?>"><?=$i?></td>
                <td height="20" align="center" bgcolor="<?=$color?>"><input type="checkbox" name="selectcheck" value="<?=$rs["photo_id"]?>"/></td>
                <td height="20" bgcolor="<?=$color?>" style="padding-left:10px;" align="left"><a href="editexpert.php?tType=Edit&uid=<?php echo $rs["photo_id"];?>" style="color:black;cursor:pointer;text-decoration:underline;" class="tt"><?echo $rs["title"]; ?><span class="tooltip"><img src="../uploads/<?php echo $rs["image"]?>" border="1" width="300" height="200"  ></span></a></td>
		...
		<input type="hidden" name="chkdelids">
          ...
          <tr>
            <td height="25" align="center" bgcolor="#F5F5F5"><?if($pg>1){?><a href="viewexpert.php?pg=1&disType=<?=$disType1?>&sortby=<?=$sortby?>" class="link3">&lt;&lt; Start</a><?}else{?><span class="link3">&lt;&lt; Start</span><?}?>&nbsp;&nbsp;<?if($pg>1){?><a href="viewexpert.php?pg=<?=$pg-1?>&disType=<?=$disType1?>&sortby=<?=$sortby?>" class="link3">&lt; Previous&nbsp;</a><?}else{?><span class="link3">&lt; Previous&nbsp;</span><?}?> | <?if($intpagecount>$pg){?>&nbsp;<a href="viewexpert.php?pg=<?=$pg+1?>&disType=<?=$disType1?>&sortby=<?=$sortby?>" class="link3">Next &gt;</a><?}else{?>&nbsp;<span class="link3">Next &gt;</span><?}?> <?if($intpagecount>$pg){?>&nbsp;<a href="viewexpert.php?pg=<?=$intpagecount?>&disType=<?=$disType1?>&sortby=<?=$sortby?>" class="link3">End &gt;&gt;</a><?}else{?>&nbsp;<span class="link3">End &gt;&gt;</span><?}?></td>
          </tr>
		  <?}else{?>
		  <tr>
...
{
$qry=mysql_fetch_array(mysql_query("select title from wn_photogallery where photogallery_id='".$id."'"));
return $qry['title'];
}
