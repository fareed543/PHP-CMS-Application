...
$count_que="select count(*) from  wn_service";
...
if(isset($_GET["va"])){
	switch($_GET["va"]){
		case '1':
			$message="New Video details Added";
			break;
		case '2':
			$message="Selected Video details Modified";
			break;
		case '3':
			$message="Selected Video details Deleted";
			break;
		case '4':
...
if(!isset($sortby)) { $sortby="se_name";}
...
$strSql="select * from  wn_service order by $sortby $disType limit $limit1,$pagelimit";
...
function fnAdd()
{
	window.location.href="editbusiness.php?tType=Add";
}
function fnDetails()
{
	...
	{
		window.location.href="editbusiness.php?tType=Edit&pg=<?=$pg?>&uid="+arrval[0];
	}
	}
...
{	
	...
	}else if(flag==1)
	{
			var i,len,chkdelids,sep;
			  ...
			  sep="";
				...
				ConfirmStatus = confirm("Do you want to DELETE selected Video?")

                if (ConfirmStatus == true) {

				document.frmMain.chkdelids.value=chkdelids
				document.frmMain.tType.value="Del"
				document.frmMain.action="submitbusiness.php";
  ...
  <tr>
    <td width="25%" height="40" align="left" valign="middle"><table border="0" cellspacing="0" cellpadding="0" style="margin-left:30px;">
	  <tr>
        <td height="40" ><span class="innerpageheading">Content Mangement : </span><span class="innerpageheading1">Video Gallery</span></td>
      </tr>
    </table></td>
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0">
          <tr>
            <td height="25" bgcolor="F5F5F5"><table width="100%" height="25" border="0" cellpadding="1" cellspacing="1">
              <tr>
                <td width="25%" align="left" bgcolor="#F5F5F5" style="padding-left:25px;"><strong>Videos ( <?=$total_count?> )</strong> </td>
		  ...
		  <tr>
            <td valign="top"><table width="100%" border="0" cellpadding="5" cellspacing="1" bgcolor="#E2E2E2" style="border-bottom:1px solid #D2001E;">
              <tr>
                ...
                <td height="45%" align="center" valign="middle" background="images/tab_img_bg.gif" class="tableheading"><a href="<?=$myurl."&sortby=se_name"?>" class="link3">Video Name</a> </td>
                 <!-- <td width="21%" height="65%" align="center" valign="middle" background="images/tab_img_bg.gif" class="tableheading">status</td>
 -->                
               </tr>
			  <?
				$i=1;
              ...
              <tr>
				...
				<input type="checkbox" name="selectcheck" value="<?=$rs["se_id"]?>"/></td>
                <td height="20" bgcolor="<?=$color?>" style="padding-left:10px;" align="left"><a href="editvill.php?tType=Edit&pg=1&uid=<?=$rs["se_id"]?>" style="color:black"><?=$rs["se_name"]?></a></td>
					 <!-- <td height="20" bgcolor="<?=$color?>" style="padding-left:10px;" align="left"><?php 
					 //echo $rs["im_top"];
					 if($rs["im_top"] =='Y'){
					echo'<img src="images/yes_icon.gif" height="10" width="12" />';
					 } else {
					echo'<img src="images/no_icon.gif" height="10" width="12" />';
					 }
		  ...
		  <tr>
            <td valign="top"><table width="100%" border="0" cellpadding="5" cellspacing="1" bgcolor="#E2E2E2" style="border-bottom:1px solid #D2001E;">
              ...
              <tr>
                <td height="100" align="center" bgcolor="#FFFFFF" colspan="9" class="pagingbold">Currently No Videos To Display</td>
              </tr>
            </table></td>
          </tr>
		  <?}?>
		...
		<input type="hidden" name="chkdelids">
          ...
          <tr>
            <td height="25" align="center" bgcolor="#F5F5F5"><?if($pg>1){?><a href="viewbusiness.php?pg=1&disType=<?=$disType1?>&sortby=<?=$sortby?>" class="link3">&lt;&lt; Start</a><?}else{?><span class="link3">&lt;&lt; Start</span><?}?>&nbsp;&nbsp;<?if($pg>1){?><a href="viewbusiness.php?pg=<?=$pg-1?>&disType=<?=$disType1?>&sortby=<?=$sortby?>" class="link3">&lt; Previous&nbsp;</a><?}else{?><span class="link3">&lt; Previous&nbsp;</span><?}?> | <?if($intpagecount>$pg){?>&nbsp;<a href="viewbusiness.php?pg=<?=$pg+1?>&disType=<?=$disType1?>&sortby=<?=$sortby?>" class="link3">Next &gt;</a><?}else{?>&nbsp;<span class="link3">Next &gt;</span><?}?> <?if($intpagecount>$pg){?>&nbsp;<a href="viewbusiness.php?pg=<?=$intpagecount?>&disType=<?=$disType1?>&sortby=<?=$sortby?>" class="link3">End &gt;&gt;</a><?}else{?>&nbsp;<span class="link3">End &gt;&gt;</span><?}?></td>
          </tr>
		  <?}else{?>
		  <tr>
