...
if($_REQUEST["tType"]=="Add")
{
	...
	$sqlstr = "insert into wn_expert ( title,link,image) values (  '".$_REQUEST['title']."','".$_REQUEST['link']."','".addslashes($image3)."' )";
	 
	mysql_query($sqlstr);
	@header("location: viewexpert.php?va=1");
	exit;
}
...
//exit;
	...
	$sqlstr = "update wn_expert set  title='".$_REQUEST['title']."', link='".$_REQUEST['link']."', image='".$image3."'  where photo_id=".$_REQUEST["uid"];
...
//exit;
	...
	if($_REQUEST['app']==1)
	{
		header("location: editexpert.php?tType=Edit&pg=$pg&uid=$_REQUEST[uid]");
		exit;
	}else
	{
		header("location: viewexpert.php?va=2&pg=$pg");
		exit;
	}
...
else if($_REQUEST["tType"]=="Del")
{
		...
		for($i=0;$i<$count;$i++)
		{
		$query = "select image from wn_expert where photo_id=".$messageid[$i];
			//echo $query;
			//exit;
		...
		$sqlstr = "delete from  wn_expert where photo_id=".$messageid[$i];
		//echo $sqlstr;
		//exit;
		mysql_query($sqlstr);
		}
	header("location: viewexpert.php?va=3&pg=$pg");
	exit;
}
?>