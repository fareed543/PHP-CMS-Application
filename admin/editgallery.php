...
{
	$sqlstr = "select * from wn_photogallery where photogallery_id=".$_REQUEST["uid"];
	$myResult = mysql_query($sqlstr);
	if($rs = mysql_fetch_array($myResult))
	{
		//$ename		= $rs["e_name"];
		//$photogallery_id  =stripslashes($rs["photogallery_id"]);
...
function fnValidate(id)
{
	...
	if(id==1)
	{
		document.frmFinal.action="submitgallery.php?app="+id;
	}
	else
	{
		document.frmFinal.action="submitgallery.php?app="+id;
...
function fnform(){
if(document.frmFinal.title.value==''){
		alert("please enter title");
		document.frmFinal.title.focus();
		return false;
	}
else return true;
}
              ...
              <tr>
                ...
                <tr>
                    ...
                    <td height="25" colspan="2" align="left">
                      <input name="title" type="text" class="form" value="<?=$title?>" size="60" onclick="fnform()"/></td>
                  </tr>
