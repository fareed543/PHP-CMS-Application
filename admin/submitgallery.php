...
if($_REQUEST["tType"]=="Add")
{
	...
	$sqlstr = "insert into wn_photogallery (title) values ('".$_REQUEST["title"]."')";
	//echo $sqlstr;
	//exit;
	mysql_query($sqlstr);
	$ID = mysql_insert_id();
/*		mysql_query("update ap_careers set article_image='".$frontfile1."' where article_id=".$ID);
	...
	if($_REQUEST['app']==1)
	{
		header("location: editgallery.php?tType=Edit&pg=$pg&uid=$ID");
		exit;
	}elseif($_REQUEST['app']==0)
	{
	header("location: viewgallery.php?va=1&sp=sub8");
	exit;
	}
...
/*	$date=explode("-",$_REQUEST["fdate"]);
	$date1= $date[2]."-".$date[1]."-".$date[0];
	 ...
	 if (is_uploaded_file($_FILES['article_image']['tmp_name']))
		...
		*/
	$sqlstr = "update wn_photogallery set title='".$_REQUEST["title"]."' where photogallery_id=".$_REQUEST["uid"];
	//echo $sqlstr;
	//exit;
	mysql_query($sqlstr);
	if($_REQUEST['app']==1)
	{
		header("location: editgallery.php?tType=Edit&pg=$pg&uid=$_REQUEST[uid]");
		exit;
	}else
	{
		header("location: viewgallery.php?va=2&pg=$pg");
		exit;
	}
...
else if($_REQUEST["tType"]=="Del")
{
		...
		for($i=0;$i<$count;$i++)
		{
			...
			$sqlstr = "delete from wn_photogallery where photogallery_id=".$messageid[$i];
			mysql_query($sqlstr);
		}
	header("location: viewgallery.php?va=3&sp=sub8&pg=$pg");
	exit;
}
